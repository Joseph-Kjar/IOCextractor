...
#Looks for indicators that may be split with newlines
def tag_prep():
    
    #md5
    lineCount = int(text.index('end-1c').split('.')[0])  # returns line count
    text.tag_configure('md5', background='#FE6AA8')
    linenumber = 1
    for linenumber in range(1, lineCount):
        twoLines = text.get('%d.0' % linenumber, '%d.end+1l' % linenumber)
        for m in re.finditer(reMD5n, twoLines, re.IGNORECASE):
            x = text.index(str(linenumber) + '.' + str(m.start()))
            y = text.index(str(linenumber+1) + '.' + str(m.end()))
            tag_new('md5', x, y)
        linenumber += 1
...
def tag_new(tag, x=None, y=None):
    if x!=None and y!=None:
        #remove any newline characters from split indicators
        if '\n' in text.get(x, y):
            line_start = (str(x).split('.'))[1]
            newline_index = text.get(x, y).index('\n')
            del_line = str(x).split('.')[0]
